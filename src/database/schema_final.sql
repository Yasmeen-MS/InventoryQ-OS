-- ResQ OS Database Schema - FIXED & VERIFIED
...
CREATE OR REPLACE TABLE inventory_master (
    ...
    reorder_quantity NUMBER(10,2) NOT NULL, -- Threshold in UNITS
    critical_days NUMBER(10,2) NOT NULL,    -- Threshold in DAYS
    last_updated TIMESTAMP_NTZ DEFAULT CURRENT_TIMESTAMP(),
    -- Expanded Location Columns
...
-- FIXED: Insert into CORRECT columns (not 'location_data') and fixed logic columns
INSERT INTO inventory_master (
    inventory_id, organization_id, sector_type, item_type, 
    current_stock, daily_consumption_rate, reorder_quantity, critical_days,
...
SELECT 
    ...
    CASE 
        ...
        ELSE ROUND(i.current_stock / i.daily_consumption_rate, 1)
    END as days_remaining,
    CASE 
        -- Critical Check: Time based
        WHEN (i.current_stock / NULLIF(i.daily_consumption_rate, 0)) <= i.critical_days THEN 'CRITICAL'
        -- Warning Check: Quantity based
        WHEN i.current_stock <= i.reorder_quantity THEN 'WARNING'
        ELSE 'NORMAL'
    END as status,
